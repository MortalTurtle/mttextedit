...
class View:
    ...
    def _paint_range(self, text_lines, color, top, bot):
        ...
        for y in range(min_y + 1, max_y):
            self._draw_single_selected_line(
                text_lines, color, y, self._offset_x, self._offset_x + width - 1)
    ...
    def _draw_user_positions(self, text_lines, user_positions, users_shift_pos, max_username_len=None):
        height, width = self.stdscr.getmaxyx()
        if max_username_len:
    ...
    def _draw_changes(self, text_lines, changes_frames):
        for frame in changes_frames:
            ...
            self._paint_range(
                text_lines, color, frame[1], frame[2])

    def draw_text(self, text_lines, user_positions, users, users_shift_pos, changes_frames=None):
        height, width = self.stdscr.getmaxyx()
