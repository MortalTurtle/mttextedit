...
class TestCLIModule(unittest.TestCase):
    ...
    def test_get_permissions_not_exists(self, mock_makedirs, mock_open):
        permissions = core.cli.get_permissions()
        self.assertEqual(permissions, {})
        mock_makedirs.assert_called_once_with(
            os.path.dirname(core.cli.PERMISSION_FILE), exist_ok=True
        )
    ...
    def test_manage_permissions_add(self, mock_get, mock_open):
        result = core.cli.manage_permissions("user2", "+rw")
    ...
    def test_connect_to_session_valid(self, mock_re, mock_app):
        ...
        core.cli.connect_to_session(True, "192.168.0.1", "test_user")
    ...
    def test_connect_to_session_invalid(self, mock_re, mock_print):
        ...
        core.cli.connect_to_session(False, "invalid", "user")
    ...
    def test_host_session_failure(self, mock_open, mock_print):
        core.cli.host_session(False, "missing.txt", "user")
    ...
    def test_list_all_saved_history(
        ...
        core.cli.list_all_saved_history("/path/to/file.txt")
        mock_makedirs.assert_called_once_with(
            os.path.dirname(core.cli.HISTORY_FILE_PATH + "/file.txt/"),
            exist_ok=True
        )
        self.assertEqual(mock_print.call_count, 2)
    ...
    def test_show_changes_success(self, mock_listdir, mock_app):
        core.cli.show_changes("/path/to/file.txt", "1")
    ...
    def test_show_blame_success(self, mock_listdir, mock_app):
        core.cli.show_blame("/path/to/file.txt", "1")
