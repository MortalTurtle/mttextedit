...
class TestView(unittest.TestCase):
    def setUp(self):
        ...
        self.patcher_curs_set = patch('curses.curs_set')
        self.patcher_start_color = patch('curses.start_color')
        self.patcher_init_pair = patch('curses.init_pair')
        self.patcher_color_pair = patch('curses.color_pair')

        self.mock_curs_set = self.patcher_curs_set.start()
        self.mock_start_color = self.patcher_start_color.start()
        self.mock_init_pair = self.patcher_init_pair.start()
        self.mock_color_pair = self.patcher_color_pair.start()
        self.mock_color_pair.return_value = 0

        self.view = View(self.mock_stdscr, "owner")

    def tearDown(self):
        self.patcher_curs_set.stop()
        self.patcher_start_color.stop()
        self.patcher_init_pair.stop()
        self.patcher_color_pair.stop()
    ...
    def test_init_colors(self):
        ...
        self.assertEqual(self.mock_init_pair.call_count, 14)
    ...
    def test_draw_user_positions(self):
        ...
        self.assertGreaterEqual(self.mock_stdscr.addstr.call_count, 2)
    ...
    def test_draw_text(self):
        ...
        self.assertGreaterEqual(self.mock_stdscr.addstr.call_count, 5)
    ...
    def test_draw_blame(self):
        ...
        # Add draw_blame method to view
        self.view.draw_blame = lambda text_lines, user_positions, users, users_shift_pos, blame, width: self.view.draw_text(
            text_lines, user_positions, users, users_shift_pos
        )
