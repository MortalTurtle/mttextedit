...
class TestView(unittest.TestCase):
    ...
    def test_draw_user_shift_pos(self):
        ...
        top = (0, 0)
        bot = (1, 1)
        self.view._user_color_index = {user: 5}  # задаём индекс цвета пользователя
        with patch('curses.color_pair') as mock_color_pair, \
    ...
    def test_draw_user_positions(self):
        ...
        with patch('curses.color_pair') as mock_color_pair, \
            ...
            self.mock_stdscr.addstr.assert_any_call(2 - self.view._offset_y, 2 - self.view._offset_x, "line2", 3)
            # Проверка, что addstr был вызван для user1 с пробелом
            self.mock_stdscr.addstr.assert_any_call(1 - self.view._offset_y, 1 - self.view._offset_x, " ", 2)
